...
class RealBasicVSRNet(nn.Module):
    def __init__(self, num_feat=64, num_block=10, spynet_path=None):
        ...
        self.backward_trunk_1 = \
            ConvResidualBlocks(num_feat, num_feat // 2, num_block)
        ...
        # feat_1 + b1 + b2 + shuffle feat_prop_1: 32 + 32 + 32 + 32 channels
        # f1
        self.forward_trunk_1 = \
            ConvResidualBlocks(num_feat * 3 // 2, num_feat // 2, num_block)

        # feat_2 + b1 + b2 + shuffle feat_prop_2: 32 + 32 + 32 + 32 channels
        # f2
    ...
    def propagate_back(self, feat_1, feat_2, flows: torch.Tensor):
        ...
        for i in range_:
            ...
            # Channel shuffle 2 flow propagation branches
            feat = torch.cat([feat_prop_1, feat_prop_2], dim=1)
            feat = self.channel_shuffle(feat)
            feat = torch.split(feat, 32, dim=1)

            # To tensor shape: [b, 64, 64, 64]
            feat_grid_1 = torch.cat([feat_i_1, feat[0]], dim=1)
            feat_grid_2 = torch.cat([feat_i_2, feat[1]], dim=1)
            ...
            # feat: [2, 64, 64, 64]
            # Channel shuffle 2 Backward propagation branches for next steps
            # Re-create feat to update the results
            # Online shuffle for backward propagation
            feat = torch.cat([feat_prop_1, feat_prop_2], dim=1)
            feat = self.channel_shuffle(feat)
            feat = torch.split(feat, 32, dim=1)

            out_prop_1.append(feat[0])
            out_prop_2.append(feat[1])

        return out_prop_1, out_prop_2
    
    def propagate_for(self, feat_1, feat_2, shuffle_b1, shuffle_b2, flows: torch.Tensor):
        b, n, _, h, w = feat_1.size()
        ...
        for i in range_:
            ...
            feat_grid_1 = torch.cat([feat_i_1, feat[0], shuffle_b1[i]], dim=1)
            feat_grid_2 = torch.cat([feat_i_2, feat[1], shuffle_b2[i]], dim=1)

            # Forward cross propagation
    ...
    def forward(self, lqs: torch.Tensor, return_lqs=False):
        ...
        shuffled_b1, shuffled_b2 = self.propagate_back(feat_1, feat_2, flows_backward)
        shuffled_b1.reverse()
        shuffled_b2.reverse()
        f1, f2 = self.propagate_for(feat_1, feat_2, shuffled_b1, shuffled_b2, flows_forward)

        out_l = []
        for i in range(0, n):
            out = torch.cat([shuffled_b1[i], shuffled_b2[i], f1[i], f2[i]], dim=1)
